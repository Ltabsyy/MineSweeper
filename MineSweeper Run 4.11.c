...
/*struct EnumResult
{
	...
	int** dictionary;
	int* sumDictionary;
	int* sumMineOfPossibility;
	int numberOfThought, numberOfPossibility;
	int minNumberOfMine, maxNumberOfMine;
};
struct EnumResults
{
	struct EnumResult* thinkChain;
	int numberOfThinkChain;
...
int countOfDictionaryOverflow = 0;
...
int WholeThink()
{
	...
	while(1)
	{
		...
		for(c=0; c<numberOfThought; c++)
		{
			result.sumDictionary[c] = possibility[c];
		}
		for(i=0; i<realNumberOfPossibility; i++)
		{
			result.sumMineOfPossibility = 0;
			for(c=0; c<numberOfThought; c++)
			{
				result.sumMineOfPossibility += dictionary[r][c];
			}
		}
		for(r=0; r<heightOfBoard; r++)
		{
			for(c=0; c<widthOfBoard; c++)
			{
				...
				result.dictionary[r][c] = dictionary[r][c];
			}
		}
...
void Bench(int seedMin, int seedMax, int r0, int c0, int showStep, int showSolution)//Bench模块
{
	...
	if(debug == 1 || debug == 2)
	{
		...
		printf("放弃链数：%d 字典溢出：%d\n", numberOfAbandonedThinkChain, countOfDictionaryOverflow);
		/*for(i=0; i<128; i++)
		{
