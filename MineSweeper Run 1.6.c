...
{
	if(backgroundColor == 0xf0) color += 0xf0;//浅色模式
...
int main()
{
	...
	int showTime = 0, t0, t1, yOfMapEnd;//显示用时
	int operateMode = 0, ro, co;//操作模式和上一个操作坐标，0@#rc，1wasd23
	int set, temp;//设置
	...
	while(1)//main内循环防止变量重复定义
	{
		...
		if(choiceMode == 1)
		{
			...
			while(1)
			{
				...
				t1 = time(0);
				if(showTime == 1) printf("用时：%d\n", t1-t0);
				if(operateMode == 0) printf("选择模式与坐标(@:翻开/#:标记)\n[@/#] [r] [c]\n");
				if(operateMode == 1) printf("选择模式与坐标\n[wasd:移动/2:翻开/3:标记]\n");
				/*输入和显示用时*/
				printf(">");
				operation = 0;
				r = ro;
				...
				while(1)
				{
					if(kbhit())
					{
						...
						if(operateMode == 1)//wasd操作设计
						{
							...
							if(key == '2') operation = '@';//2翻开
							if(key == '3') operation = '#';//3标记
				...
				else if(operation == '!')//游戏时遇到难解局面，输入! 0 0
				{
			...
			while(1)
			{
				...
				else if(set == 2)//调试选项
				{
					printf("*******************************\n"//宽31
						   ...
						   "(3)以密度设置雷数\n"
						   "(4)设置地图生成校验\n");
					if(showTime == 0) printf("(5)启用游戏时实时显示用时\n");
					if(showTime == 1) printf("(5)关闭游戏时实时显示用时\n");
					if(backgroundColor == 0x07) printf("(6)浅色模式\n");
					if(backgroundColor == 0xf0) printf("(6)深色模式\n");
					if(operateMode == 0) printf("(7)更改为wasd23操作模式\n");
					if(operateMode == 1) printf("(7)更改为@#rc操作模式\n");
					printf("*******************************\n");
					getchar();
					printf(">");
					operation=getchar();
					...
					else if(operation == '5')//更改游戏结束是否显示用时
					{
						if(showTime == 0)
						{
							showTime = 1;
