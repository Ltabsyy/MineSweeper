			...
			while(1)
			{
				...
				while(1)
				{
					if(kbhit())//按键
					{
						...
						//操作模式切换
						if((key == '@' || key == '#' || key == '!' || key == '$') && operateMode == 1)
						{
		...
		else if(choiceMode == 3)
		{
			while(1)
			{
				...
				if(set == 1)//设置游戏难度
				{
					...
					else if(difficulty == 4)//顶级，胜率13%
					{
						numberOfMine = 715;//密度0.1986111
						heightOfBoard = 40;
						widthOfBoard = 90;//实测较为合适的全屏地图，雷数为胡桃生日
					}
					else
					{
						/*设置雷数*/
		...
		else if(choiceMode == 4)
		{
			...
			for(seed=seedMin; seed<=seedMax; seed++)
			{
				...
				while(1)
				{
					...
					Solve();
					debug=temp;
					if((debug == 1 || debug == 2) && difficulty != 4)//Bench仅显示方案矩阵
					{
						printf("[Debug]已生成方案矩阵：");
