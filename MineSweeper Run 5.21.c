...
void MapSearch(int seedMin, int seedMax, int r0, int c0, int iterateMax)//地图搜索模块
{
	...
	else if(choice == 3)//3BV汇总统计
	{
		...
		printf("\rseed=%d 用时：%d 平均用时：%.2f ms\n", seedMax, t1-t0, (float)(t1-t0)/(seedMax-seedMin+1)/iterateMax*1000);
	...
	else if(choice == 6)//地图生成效率
	{
		...
		printf(" 平均用时：%.2f ms\n", (float)(t1-t0)/(seedMax-seedMin+1)/iterateMax*1000);
		showCursor(1);
	}
