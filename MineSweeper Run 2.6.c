...
//void DrawBlock(int r, int c, int board, int isShown);
//void DrawBoard(int mode);//在外部窗口显示地图
...
int operateMode = 2;//操作模式，0@#rc，1wasd23，2鼠标点击
...
void DrawSetDifficulty(int select)
{
	...
	gotoxy(0, 0);
	printf("**************************************************************\n");//宽62
	printf("** [(0)默认：10*10 - 10] **\n");
	printf("** [(1)初级： 9*9  - 10] **\n");
	printf("** [(2)中级：16*16 - 40] **\n");
	printf("** [(3)高级：16*30 - 99] **\n");
	printf("** [(4)顶级：42*88 -715] **\n");
	printf("** [(5)自定义***** - **] **\n");
	printf("**************************************************************\n");
	if(select == 0)
	{
...
void DrawBlock(int r, int c, int board, int isShown)
{
	...
	if(isShown == 1 && board != 0)
	{
		...
		{
			setcolor(BLUE);
			xyprintf(c*widthOfBlock+xOfChar, r*heightOfBlock+yOfChar, "1");
		}
		else if(board == 2)
		{
			setcolor(GREEN);
			xyprintf(c*widthOfBlock+xOfChar, r*heightOfBlock+yOfChar, "2");
		}
	...
	else if(isShown == 2)
	{
		...
		setfillcolor(BLACK);
		//绘制底座
		ege_fillrect(c*widthOfBlock+8, r*heightOfBlock+24, 16, 2);
		ege_fillrect(c*widthOfBlock+10, r*heightOfBlock+22, 12, 2);
		//绘制旗杆
		ege_fillrect(c*widthOfBlock+15, r*heightOfBlock+16, 2, 8);
		//绘制旗帜
		setfillcolor(RED);
		ege_point polyPoints[3] =
		{
			{c*widthOfBlock+6, r*heightOfBlock+11},
			{c*widthOfBlock+17, r*heightOfBlock+6},
			{c*widthOfBlock+17, r*heightOfBlock+16}
		};
...
void DrawBoard(int mode)
{
	...
	for(r=0; r<heightOfBoard; r++)
	{
		for(c=0; c<widthOfBoard; c++)
		{
			...
			{
				DrawBlock(r, c, board[r][c], isShown[r][c]);
			}
		}
	}
}
