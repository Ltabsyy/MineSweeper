...
int main()
{
	...
	//SetConsoleTitle("MineSweeper Run");
					...
					while(1)//简易版wasd23
					{
						if(operateMode == 2)
						{
							...
							if(rcd.EventType == MOUSE_EVENT)
							{
								...
								if(mousePos.Y-rs0 >= 0 && mousePos.Y-rs0 < heightOfBoard
									&& (mousePos.X-cs0)/2 >=0 && (mousePos.X-cs0)/2 < widthOfBoard)
								{
									r0 = mousePos.Y-rs0;
									c0 = (mousePos.X-cs0)/2;
				...
				while(1)
				{
					if(operateMode == 2)
					{
						...
						if(rcd.EventType == MOUSE_EVENT)
						{
							...
							if(mousePos.Y-yOfMapEnd+heightOfBoard-1 >= 0 && mousePos.Y-yOfMapEnd+heightOfBoard-1 < heightOfBoard
								&& (mousePos.X-cs0)/2 >=0 && (mousePos.X-cs0)/2 < widthOfBoard)
							{
							...
							/*if(mousePos.X == 0 && mousePos.Y-yOfMapEnd+heightOfBoard-1 < 0)//鼠标全选复制地图
							{
								if(rcd.Event.MouseEvent.dwButtonState == FROM_LEFT_1ST_BUTTON_PRESSED)
								{
									SetConsoleMode(hdin, ENABLE_QUICK_EDIT_MODE | ENABLE_EXTENDED_FLAGS);
