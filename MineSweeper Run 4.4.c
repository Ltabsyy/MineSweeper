...
int main()
{
	...
	int visibleCursor = 1;//1显示光标，0隐藏光标
					...
					if(operateMode > 0)
					{
						...
						if(newCursor > 0)
						{
							if(newCursor > 1)//绘制淡黄色高亮光标
							{
								...
								{
									//ReShowBoard(-1, -1, 0, yOfMapEnd-heightOfMapShown+1);//重绘全图
									ReShowBoard(ra, ca, 0, yOfMapEnd-heightOfMapShown+1);
									ReShowBoard(r, c, 2, yOfMapEnd-heightOfMapShown+1);
									if(newCursor == 3)//淡黄色高亮行列
									{
										if(ra != r) ReShowBoard(ra, -1, 0, yOfMapEnd-heightOfMapShown+1);
										if(ca != c) ReShowBoard(-1, ca, 0, yOfMapEnd-heightOfMapShown+1);
							...
							else if(newCursor == 1)//绘制><光标
							{
								...
								for(ra=r-2; ra<=r+2; ra++)
								{
									for(ca=c-2; ca<=c+3; ca++)//向右偏移
									{
										...
										{
											gotoxy(cs0+2*ca-1, yOfMapEnd-heightOfBoard+1+ra);
											printf(" ");//去除左侧拖影
										}
									}
								}
			...
			while(choiceMode == 3 && choiceSet != 7)
			{
				...
				else if(choiceSet == 6)//调试选项
				{
					printf("*******************************\n"//宽31
						   ...
						   "(4)设置背景颜色\n"
						   "(5)设置刷新周期\n");
					if(fastSign == 0) printf("(6)启用#数字快速标记周围\n");
					if(fastSign == 1) printf("(6)关闭#数字快速标记周围\n");
					printf("(7)设置光标\n");
					...
					else if(operation == '7')//设置光标
					{
						...
						if(visibleCursor == 0) printf("(1)显示控制台光标   当前:隐藏\n");
						if(newCursor == 0) printf("(2)设置自选光标形态 当前:未启用\n");
						if(newCursor == 1) printf("(2)设置自选光标形态 当前:><光标\n");
						if(newCursor == 2) printf("(2)设置自选光标形态 当前:淡黄色高亮光标\n");
						if(newCursor == 3) printf("(2)设置自选光标形态 当前:淡黄色高亮行列\n");
						printf("(3)退出\n"
							   "*******************************\n");
						printf(">");
						scanf("%d", &temp);
						...
						else if(temp == 2)
						{
							...
							printf(">");
							scanf("%d", &temp);
							if(temp >= 0 && temp <= 3) newCursor = temp;
							//自动隐藏乱动的控制台光标
...
void DrawSettingsBar(int select)
{
	...
	else if(fastSign == 1) printf("* [(6)禁用快速标记]      当前:启用\n");
...
int GamerLevel(struct Records records)//计算玩家等级并显示称号
{
		...
		&& records.minimumTime[2] != -1 && records.minimumTime[3] != -1)
	{
			...
			&& records.minimumTime[3] <= 243)//242.64秒内赢高级地图
		{
			...
			{
				level = 6;//"ProGamer*"(Ltabsyy: 5 7 45 155 1656)
