...
（4）用时以秒级计算而非毫秒级（包括时间纪录、3BV/s、SS）
...
进入设置页面并进入调试选项，输入9回车，再输入1回车，点击返回并保存，即可启用保存连续（未暂停或重启程序）且有效记录的操作记录。

在主页按V，将操作记录文件拖入并回车，即可播放。
...
数对之二可能在数对之一的5*5共24个方块的范围内，其中上下左右4个方块称为常形逻辑解，剩余20个方块称为异形逻辑解。
...
雷率解是不确定解，但结合实际地图又是确定解；Run 1.4-4.10版本可解地图均不排除雷率解，Run 4.11开始排除雷率解。
...
可解地图按生成方式分为筛选式和调整式，筛选式适用于低密度、小地图难度，调整式适用于高密度、大地图难度。

Run 1.4-5.22版本仅存在筛选式可解地图生成算法，Run 5.23开始试行引入调整式。

筛选式算法性能高效，从Run 5.22开始，在默认地图求解器设置下，所有预设难度（包括顶级）都可在0.1秒以内生成。

调整式算法当前仅供体验，不支持操作记录（录像）、中断（暂停或重启程序）的情况，且仅供自定义难度使用。

在设置页面进入调试选项，输入2回车，再输入6回车，点击返回并保存，即可体验调整式可解地图。
...
其产生的游戏纪录仅用作自我提升，不可与其他扫雷游戏程序的纪录横比。
