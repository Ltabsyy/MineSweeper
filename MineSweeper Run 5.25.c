...
//#define kbhit _kbhit
//#define getch _getch
//#define mkdir _mkdir//vs没救了，必须关闭SDL检查
#include <windows.h>//面向Windows输出彩色字符
#include <math.h>
#include <direct.h>//创建文件夹
//#define SHOW_CONSOLE//vs专属(doge)
...
//内存占用至多约为 79*H*W+L*D Byte，默认约1.3MB
...
int WinByRightSign();//判断所有雷都被标记且无错误标记，可存在非雷未翻开，用于仅标记雷可赢
...
int IsSolvableMap(int r0, int c0);
void SwapMineUpdateBoard(int r1, int c1, int r2, int c2);
void AdjustSolvableMap(int seed, int r0, int c0);//生成调整式可解地图
...
void AddOperationFast(struct Operations* operations, int mstime, char operation, int r, int c);
...
void WriteOperations(struct Operations operations, struct Record record);
char* InputFileName();
...
int BBBV(int mode);//Bechtel'sBoardBenchmarkValue，计算地图3BV
...
void MapSearch(int seedMin, int seedMax, int r0, int c0, int iterateMax);//地图搜索模块
...
int** isSolved = 0;//已解区，一般1标记周围无未知方块的非0数字
...
int summonCheckMode = 3;//0不校验，1非雷，2必空，3可解，4筛选
...
int benchContinuousSolve = 0;//Bench启用连续求解加速
...
int adjustedSolvableMap = 0;//0可解地图使用筛选式，1调整式(仅自定义难度)
...
//int diffResult;
int asmcSolve, asmcChise, asmcRearrange, asmcSwing;//调整式可解地图统计
...
int main(int argc, char** argv)
{
	...
	{//内嵌括号减少warning，其实可以不加(doge)
		...
		fscanf(file, "benchContinuousSolve=%d\n", &benchContinuousSolve);
		...
		fscanf(file, "adjustedSolvableMap=%d\n", &adjustedSolvableMap);
		fclose(file);
	...
	while(choiceMode != 5)
	{
		...
		if(choiceMode == 1)//游戏
		{
			...
			if(lastMap == 0)
			{
				ro = r0;
				co = c0;
				operationRecord = AddOperations(seed, r0, c0);//开始记录操作记录，包括连续局、暂停局
			}
			...
			clrscr();
			if(heightOfBoard <= 40) DrawControlBar(0);//地图高度大于40时不显示控制栏
			ShowBoard(0);//第一次绘制坐标轴
			game.t0 = time(0);
			...
			while(choiceMode == 1)
			{
				...
				if(isShown[r][c] == 1)//对翻开操作做出反应
				{
					...
					else if(board[r][c] == 9)//寄
					{
						...
						game.unsolved3BV = BBBV(0);//temp暂存未解3BV
				...
				if(WinByAllOpen() || WinByRightSign())//未翻开的都是雷，或正确标记所有雷则胜利
				{
					clrscr();
					DrawControlBar(0);
					ShowBoard(1);
				...
				/*显示*/
				game.yOfMap = (heightOfBoard > 40) ? 0 : 3;//地图高度大于40时不显示控制栏
				gotoxy(0, game.yOfMap);
				...
				else if(operateMode == 1) printf("[wasd:移动/2:翻开/3:标记]\n");//宽24
				else if(operateMode == 0) printf("[@:翻开/#:标记] [r] [c]  \n");//宽22+2
				/*备份至文件*/
				...
				while(choiceMode == 1)
				{
					...
					if(operateMode < 2 && kbhit())//按键
					{
						...
						if(operateMode == 0)
						{
							if(key == '@' || key == '#' || key == '$')
							{
								...
								game.clock1 = clock();
								AddOperation(&operationRecord, game.clock1-game.clock0+game.clock2, operation, r, c);
								break;
							}
						}
					...
					if(operation == '!' || operation == '\t')
					{
						...
						/*for(r=0; r<heightOfBoard; r++)
						{
							for(c=0; c<widthOfBoard; c++)
							{
								isSolved[r][c] = 0;//InitSolvedMemory();
							}
						}*/
						LookMap();
			...
			else if(choiceMode == 0)//完成地图退出
			{
				...
				newRecord.iterateMode = mapIterator;
				mapIterator = 1;
				newRecord.seed = seed;
				newRecord.r0 = r0;
				newRecord.c0 = c0;
				...
				{
					WriteOperations(operationRecord, newRecord);
				}
				ClearOperations(operationRecord);
				//if(operateMode == 3) choiceMode = CloseWindow(1-isOpenMine, "左键新游戏，右键关闭窗口");
			}
		}
		...
		else if(choiceMode == 3)//设置
		{
			...
			while(choiceMode == 3 && choiceSet != 9)
			{
				...
				else if(choiceSet == 8)//调试选项
				{
					...
					else if(operation == '2')//设置地图求解器
					{
						...
						if(adjustedSolvableMap == 0) printf("(6)启用调整式可解地图（仅自定义难度）\n");
						else printf("(6)关闭调整式可解地图\n");
						printf("(7)退出\n");
						printf("*******************************\n");
						printf(">");
						scanf("%d", &temp);
						...
						else if(temp == 2)//设置枚举限制(NC/MC)
						{
							...
							printf("[MC:%d-%d]>", lengthOfThinkNumberCheck, LimLength);
							scanf("%d", &temp);
						...
						else if(temp == 6)//调整式可解地图
						{
							if(adjustedSolvableMap == 0) adjustedSolvableMap = 1;
							else adjustedSolvableMap = 0;
						}
					}
					...
					else if(operation == '8')//设置Bench
					{
						...
						if(benchContinuousSolve == 1) printf("(5)关闭连续求解加速\n");
						else printf("(5)启用连续求解加速\n");
						printf("(6)退出\n");
						printf("*******************************\n");
						printf(">");
						scanf("%d", &temp);
						...
						{
							if(benchContinuousSolve == 1) benchContinuousSolve = 0;
							else benchContinuousSolve = 1;
						}
					}
			...
			if(choiceSet == 9)//返回并保存
			{
				...
				fprintf(file, "benchContinuousSolve=%d\n", benchContinuousSolve);
				...
				fprintf(file, "adjustedSolvableMap=%d\n", adjustedSolvableMap);
				fclose(file);
				clrscr();
				choiceMode = 0;
			}
		...
		else if(choiceMode == 4)//Bench
		{
			if(debug == 2)
			{
				...
				gotoxy(0, 1);
				printf("**                   请等待Bench结束. . .                   **\n");
				Bench(seedMin, seedMax, heightOfBoard/2, widthOfBoard/2, temp, benchShowSolution, benchShowInformation);//Bench模块
				system("pause");//暂停观察
		...
		else if(choiceMode == 6)//地图搜索
		{
			SetConsoleMouseMode(0);
			printf("当前时间戳：%d\n", (int)time(0));
			printf("[seedMin] [seedMax] [r0] [c0] [iterateMax]\n>");
			showCursor(1);
			scanf("%d%d%d%d%d", &seedMin, &seedMax, &r0, &c0, &temp);
			MapSearch(seedMin, seedMax, r0, c0, temp);//MapSearch模块
			system("pause");
			showCursor(visibleCursor);
			SetConsoleMouseMode(1);
			clrscr();
		...
		else if(choiceMode == 8)//自定义种子游戏
		{
			...
			while(scanf("%d", &mapIterator) == 0) getchar();
			lastMap = 1;
			game.t2 = 0;
			game.clock2 = 0;
			ro = r0;
...
/*
int PreviewMap()//地图求解前置校验，检查0链打开不全、明显错误标记、明显非法数字
{
	int r, c;
	for(r=0; r<heightOfBoard; r++)
	{
		for(c=0; c<widthOfBoard; c++)
		{
			if(isShown[r][c] == 1)//通过数字检查
			{
				if(numberShown[r][c] == 0)
				{
					if(NumberOfNotShownAround(r, c) != 0)
					{
						//0链打开不全
						printf(":(\n>@ %d %d\n", r, c);
					}
					if(NumberOfSignAround(r, c) != 0)
					{
						//0周围错误标记
						printf(":(\n># %d %d\n", r, c);
					}
				}
				else if(numberShown[r][c] == 9)
				{
					//非法数字9
					printf(":(\n>? %d %d\n", r, c);
				}
				...
				{
					if(NumberOfSignAround(r, c) > numberShown[r][c])
					{
						//错误标记
						printf(":(\n># %d %d\n", r, c);
					}
					if(NumberOfNotShownAround(r, c) + NumberOfSignAround(r, c) < numberShown[r][c])
					{
						//非法地图
						printf(":(\n>? %d %d\n", r, c);
					}
...
void Solve(int showAnswer)//程序核心部分(doge)
{
	...
	for(r=0; r<heightOfBoard; r++)
	{
		for(c=0; c<widthOfBoard; c++)
		{
			...
			isSolved[r][c] = 0;//1标记周围无未知方块的非0数字
	...
	while(isSolving == 1)
	{
		...
		for(r=0; r<heightOfBoard; r++)
		{
			for(c=0; c<widthOfBoard; c++)
			{
				if(isShown[r][c] == 1 && numberShown[r][c] != 0 && isSolved[r][c] == 0)//寻找数字
				{
...
void ContinuousSolve()//对当前地图连续简单判断和逻辑推理
{
	...
	while(isSolving)
	{
		...
		for(r=0; r<heightOfBoard; r++)
		{
			for(c=0; c<widthOfBoard; c++)
			{
				if(isShown[r][c] == 1 && numberShown[r][c] != 0 && isSolved[r][c] == 0)
				{
			...
			if(isSolving == 1)
			{
				...
				for(r1=0; r1<heightOfBoard; r1++)
				{
					for(c1=0; c1<widthOfBoard; c1++)
					{
						...
						{
							//数对之二必定也isThought 2，因为数对之二周围必定有未知方块，否则简单推理即可
							for(i=0; i<4; i++)//只遍历上左右下4个方块
							{
								...
								if(isThought[r2][c2] == 2)//数对之二
								{
									//数对框架
									int rc1 = r1 < r2 ? r1 : r2;//min
									int cc1 = c1 < c2 ? c1 : c2;
									int rc2 = r1 > r2 ? r1 : r2;//max
									int cc2 = c1 > c2 ? c1 : c2;
									if(rc1 > 0) rc1--;
									if(cc1 > 0) cc1--;
									if(rc2+1 < heightOfBoard) rc2++;
...
void SummonMineRateForNotShown()
{
	...
	else if(minRemainedMine == maxRemainedMine)
	{
		isMineRateOfNotShown = aveRemainedMine / realNumberOfNotShown;
	}
	else if(aveRemainedMine <= 0)
	{
		isMineRateOfNotShown = 0.01;//雷率期望<=0，趋向翻开内部
		//isMineRateOfNotShown =(float) maxRemainedMine / realNumberOfNotShown;
	}
	else if(aveRemainedMine >= realNumberOfNotShown)
	{
		isMineRateOfNotShown = 0.99;//雷率期望>=1
	}
	else
	{
		isMineRateOfNotShown = aveRemainedMine / realNumberOfNotShown;//在(0,1)范围
	}
	//if(isMineRateOfNotShown > 1 || isMineRateOfNotShown < 0) system("pause");
	if(debug == 2)
	{
		printf("<Debug>\n");
...
int IsSolvableIsland()//判断当前地图岛上可解性
{
	...
	while(1)
	{
		if(WinByAllOpen()) break;//可能第一次就开完了
		ContinuousSolve();
		if(WinByAllOpen()) break;
		LookMap();
		temp = debug;
		debug = 0;//不显示求解信息
		Solve(0);
...
int IsSolvableMap(int r0, int c0)
{
	...
	while(1)
	{
		if(WinByAllOpen()) break;//可能第一次就开完了，不考虑直接踩雷
		ContinuousSolve();
		if(WinByAllOpen()) break;//需要性能时，一般都未解完
		//else {isOpenMine = 1; break;}//仅测试连续求解，第一次知道这里不能用逗号
		LookMap();
		temp = debug;
		debug = 0;//不显示求解信息
		Solve(0);
...
{
	int r, c;
	if(isMine[r1][c1] == isMine[r2][c2]);
	else if(isMine[r1][c1] == 0 && isMine[r2][c2] == 1)
	{
		isMine[r1][c1] = 1;
		isMine[r2][c2] = 0;
		for(r=r1-1; r<=r1+1; r++)
		{
			for(c=c1-1; c<=c1+1; c++)
			{
				if(r>=0 && r<heightOfBoard && c>=0 && c<widthOfBoard)//确认在范围内
				{
					numberOfMineAround[r][c]++;
					if(board[r][c] != 9) board[r][c]++;
				}
			}
		}
		for(r=r2-1; r<=r2+1; r++)
		...
		board[r1][c1] = 9;
		board[r2][c2] = numberOfMineAround[r2][c2];
	}
	else if(isMine[r1][c1] == 1 && isMine[r2][c2] == 0)
	{
		isMine[r1][c1] = 0;
		isMine[r2][c2] = 1;
		for(r=r1-1; r<=r1+1; r++)
		...
		board[r1][c1] = numberOfMineAround[r1][c1];
		board[r2][c2] = 9;
	}
}
...
void AdjustSolvableMap(int seed, int r0, int c0)//生成调整式可解地图
{
	...
	int isSolving, numberOfNotShown, remainedMine, needChiselWall, isSwing;
	...
	SummonBoard(seed, r0, c0, 2, 1);//生成起始点必为空地图
	ShownModeBak(1);//备份零显示方式矩阵
	if(board[r0][c0] == 0)//起始点不为空时放弃
	{
		isShown[r0][c0] = 1;
		OpenZeroChain(r0, c0);
		while(1)
		{
			...
			if(isSolving) continue;//跳到头部，解离式循环，便于中间退出
			asmcSolve++;
			//ContinuousSolve();//不枚举，标准化，效率更高
			if(WinByAllOpen()) break;//所有非雷被翻开，生成完毕
			//判断是否需要凿墙
			needChiselWall = 1;
			for(r=0; r<heightOfBoard; r++)
			{
			...
			if(needChiselWall == 1)
			{
				...
				while(1)//统计效率低，随机选择
				{
					r = rand() % heightOfBoard;
					c = rand() % widthOfBoard;
					if(isShown[r][c] == 2 && NumberOfNotShownAround(r, c) != 0)
					{
						break;
					}
				}
				//选择剩余区一个非雷方块
				n = 0;//高密度非雷方块很少，随机命中率低，先统计再选择
				for(ra=0; ra<heightOfBoard; ra++)
				{
					for(ca=0; ca<widthOfBoard; ca++)
					{
						if(isShown[ra][ca] == 0 && isMine[ra][ca] == 0)
						{
							rList[n] = ra;
							cList[n] = ca;
							n++;
				...
				ra = rList[i];
				ca = cList[i];
				SwapMineUpdateBoard(r, c, ra, ca);//交换并更新数字，1 0 -> 0 1
				isShown[r][c] = 0;
				if(debug == 2)
				{
			...
			{
				//重排未知区
				numberOfNotShown = NumberOfNotShown();
				//remainedMine = numberOfMine - NumberOfSign();
				//判断是否仅剩唯一摇摆区域
				isSwing = 0;
				if(numberOfNotShown <= 5)
				{
					isSwing = 1;//不严谨，但高效(doge)
					//30*30-450测试150次摇摆，其中仅4次误判
				}
				if(isSwing)//摇摆
				{
					if(debug == 2)
					{
						...
						//printf("remainedMine=%d\n", remainedMine);
						ShowBoard(0);
						//system("pause");
					}
					//吸雷，抗摇摆可吸雷或破开，吸雷对现有求解顺序影响更小
					n = 0;
					for(r=0; r<heightOfBoard; r++)
					{
						for(c=0; c<widthOfBoard; c++)
						{
							if(isShown[r][c] == 0 && isMine[r][c] == 0)
							{
								rList[n] = r;
								cList[n] = c;
								n++;
					...
					while(1)
					{
						r = rand() % heightOfBoard;
						c = rand() % widthOfBoard;
						if(isMine[r][c] == 1) break;
					}
					SwapMineUpdateBoard(r, c, ra, ca);
					if(debug == 2)
					{
						printf("[Debug]吸雷完毕(%d,%d)<->(%d,%d)\n", r, c, ra, ca);
						ShowBoard(0);
						//system("pause");
					}
					ShownModeBak(0);//恢复并重推
...
int ThinkNumber(int numberOfThought)//选取并思考一个未知链
{
	...
	for(i=0; i<ncc.numberOfThought; i++)
	{
		...
		free(ncc.numberCheck[i].mineCheck);
	}
				...
				&& records.minimumTime[4] != -1)//赢1场顶级地图
			{
					...
					&& records.minimumTime[4] <= 2478)//2477.04秒内赢顶级地图
				{
						...
						&& records.minimumTime[4] <= 1752)//1751.27秒内赢顶级地图
					{
						...
						{
							level = 9;//"神境"(Ltabsyy: 0 0 16 49 553)
	...
	while(1)
	{
		...
		if(operateMode > 1)
		{
			...
			while(1)
			{
				...
				showCursor(visibleCursor);
				//Sleep(refreshCycle);//此处无需延时，因为ReadConsoleInput()阻塞
			}
		}
		else
		{
...
void AddOperation(struct Operations* operations, int mstime, char operation, int r, int c)
{
	if(operation == 'm')
	{
		if(operations->tail != NULL && operations->tail->operation == 'm')//连续鼠标操作
		{
			if(operations->tail->r == r && operations->tail->c == c);//坐标相同，不保存
			else if(operations->tail->mstime == mstime)//时间相同，高频情况
			{
				operations->tail->r = r;//改写上一次坐标，鼠标消息间隔式处理，最初时间的坐标已过时
				operations->tail->c = c;
			}
			else AddOperationFast(operations, mstime, operation, r, c);
		}
		else AddOperationFast(operations, mstime, operation, r, c);//保存这一次操作记录
	}
	else
	{
		if(operation == 0) operation = '?';//移动以?记录
		if(operation == '?' && operations->r == r && operations->c == c);//?方块定位操作与当前坐标相同
		else
		{
			AddOperationFast(operations, mstime, operation, r, c);
			operations->r = r;//更新当前方块坐标
			operations->c = c;
		}
	}
}
...
void WriteOperations(struct Operations operations, struct Record record)
{
	...
	struct Operation* p;
	//更新原按秒计算的数据为按毫秒计算
	float time =(float) record.msTime/1000;
	record.speed = record.solved3BV/time;
	record.score = SS(record.heightOfBoard, record.widthOfBoard, record.numberOfMine,
					  time, record.solved3BV, record.total3BV);
	//按历史记录信息命名
	//minesweeper-operations\minesweeper-operations_42x88-715_12345.678_1234_0.123_12.345_2147483647.txt
	sprintf(fileName, "minesweeper-operations\\minesweeper-operations_%dx%d-%d%s_%.3f_%d_%.3f_%.3f_%d.txt",
		record.heightOfBoard, record.widthOfBoard, record.numberOfMine,
		(record.solved3BV != record.total3BV ? "_fail" : (record.isHelped == 1 ? "_cheat" : "")),//fail和cheat标识无效记录，自定义难度无需
		time, record.total3BV, record.speed, record.score, record.seed);
	//按操作记录信息写入
...
void DrawPathP(char operation, int r, int c)
{
	static PIMAGE cachePath;
	static int cacheSideLength = 0;
	static int x1, y1;
	int x2, y2;
	float x0 = widthOfBlock*4;
	float y0 = heightOfBlock*4;
	if(operation == 'o')
	{
		putimage_withalpha(NULL, cachePath, widthOfBorder+dx-x0, heightOfBar+widthOfBorder+dy-y0);
	}
	else if(operation == 'm')
	{
		x2 = c*sideLength/64;
		y2 = r*sideLength/64;
		setcolor(WHITE, cachePath);
		setlinewidth(sideLength/16, cachePath);
		setlinecap(LINECAP_ROUND, cachePath);
		ege_line(x0+x1, y0+y1, x0+x2, y0+y2, cachePath);
		x1 = x2;
		y1 = y2;
	}
	else if(operation == '?');
	...
	{
		setfillcolor(LIME, cachePath);
		ege_fillcircle(x0+x1, y0+y1, sideLength/8, cachePath);
	}
	else if(operation == '#')
	{
		setfillcolor(RED, cachePath);
		ege_fillcircle(x0+x1, y0+y1, sideLength/8, cachePath);
	}
	else if(operation == 'i')//输入i和起始yx坐标初始化
	{
		cachePath = newimage(widthOfBlock*(widthOfBoard+8), heightOfBlock*(heightOfBoard+8));
		ege_enable_aa(true, cachePath);
		ege_setalpha(0x00, cachePath);//全图全透明
		setfillcolor(EGERGBA(0x00, 0x00, 0x00, 0x80), cachePath);//幕布半透明
		ege_fillrect(x0, y0, (float)widthOfBlock*widthOfBoard, (float)heightOfBlock*heightOfBoard, cachePath);
		x2 = c*sideLength/64;
		y2 = r*sideLength/64;
		x1 = x2;
		y1 = y2;
...
void PlayOperations(struct Operations operations)
{
	...
	ReallocMemory(heightOfBoard, widthOfBoard, dictionaryCapacity, lengthOfThinkMineCheck);
	AdaptScreenBufferWidth();
	SummonBoard(operations.seed, operations.r0, operations.c0, operations.summonMode, operations.iterateMode);
	game.total3BV = BBBV(3);
	game.unsolved3BV = game.total3BV;
	game.remainedMine = numberOfMine;
	game.yOfMap = 3;//显示准备
	...
	game.showInformation = 1;
	for(r=0; r<heightOfBoard; r++)//快速显示
	{
		for(c=0; c<widthOfBoard; c++)
		{
	...
	/*if(operateMode == 3)
	{
		...
		{
			//delay_ms(250);
			//DrawBoard(1+operations.isWin);
			//DrawPathP('o', 0, 0);
			//DrawMouse(xm+widthOfBorder+dx, ym+heightOfBar+widthOfBorder+dy);
			//delay_ms(0);
			//getch();
			if(CloseWindow(operations.isWin, "左键或右键关闭窗口")) closegraph();//左键也关闭
		}
		else
		{
			DrawBoard(0);
...
void Bench(int seedMin, int seedMax, int r0, int c0, int showStep, int showSolution, int showInformation)//Bench模块
{
	...
	int t0, t1, countOfWin = 0, countOfStep = 0;//Bench统计
	...
	for(seed=seedMin; seed<=seedMax; seed++)
	{
		...
		while(1)
		{
			...
			if(benchContinuousSolve == 1 && showStep < 1)//中间帧及以上不能正确显示方案矩阵和结束帧地图
			{
				ContinuousSolve();
				if(WinByAllOpen())
				{
		...
		if(showTime == 1)
		{
			if(showStep != -1) DrawProgressBar(100 * (seed-seedMin+1) / (seedMax-seedMin+1));//进度条
			else if(seed % 64 == 0)//无帧快速显示进度
			{
				gotoxy(55, 1);
				printf("%3d%%", 100 * (seed-seedMin+1) / (seedMax-seedMin+1));
			}
		}
		if(showStep > 1)
		{
	...
	if(debug == 1 || debug == 2)
	{
		...
		for(seed=seedMin; seed<=seedMax; seed++)
		{
			...
			{
				printf("\n");
				if(seedMax-seedMin+1 > 50000 && (seed-seedMin)%50000 == 0)
				{
					system("pause");//超过5万局时分批输出
				}
...
int BBBV(int mode)//计算地图3BV
{
	//计算模式(基于当前board和isShown)：1计算全部3BV，0计算未解3BV，3计算全部3BV并缓存，2根据缓存计算未解3BV
	int bbbv = 0, r, c, z, temp;
	int* zeroChainCache;//缓存0链是否被打开
	static int numberOfZeroChain = 0;
	if(mode != 2) ShownModeBak(1);
	if(mode == 0)//防止标记阻碍0链打开
	{
		for(r=0; r<heightOfBoard; r++)
		{
			for(c=0; c<widthOfBoard; c++)
...
int SearchSeed(int seedMin, int r0, int c0, int difficulty)//可解和筛选种子搜索模块
{
	...
	while(isOpenMine == 1)
	{
		...
		{
			if(chosenSolvable == 1 && adjustedSolvableMap == 1 && difficulty == 5 && board[r0][c0] == 0)
			{
				AdjustSolvableMap(seed, r0, c0);
			}
			bbbv = BBBV(1);//计算3BV
			if(bbbv >= chosen3BVMin[difficulty] && bbbv <= chosen3BVMax[difficulty])//筛选地图3BV
			{
				...
				}//筛选数字0不管，可能起始点必非空但存在角空
				if(isOpenMine == 0 && chosenSolvable == 1)//筛选地图可解性
				{
					if(board[r0][c0] != 0) break;//起始点不为空，放弃
					isOpenMine = 1-IsSolvableMap(r0, c0);
				}
			}
		}
		else
		{
			if(board[r0][c0] != 0) break;//起始点不为空，放弃
			if(adjustedSolvableMap == 1 && difficulty == 5)
			{
				AdjustSolvableMap(seed, r0, c0);
			}
			isOpenMine = 1-IsSolvableMap(r0, c0);
		}
		if(isOpenMine == 1)//最终地图保持种子和迭代不变
		{
			if(mapIterator < 8) mapIterator++;//保持为1-8
			else
			{
				seed++;
				mapIterator = 1;
...
void MapSearch(int seedMin, int seedMax, int r0, int c0, int iterateMax)//地图搜索模块
{
	int seed, bbbv, iterator, temp;
	int summonMode = summonCheckMode;
	int t0, t1, t2 = 0;
	int choice = 1;
	int r, c;
	if(summonMode > 2) summonMode = 2;
	if(iterateMax < 1) iterateMax = 1;
	for(r=0; r<heightOfBoard; r++)
	{
		for(c=0; c<widthOfBoard; c++)
		{
	...
	else if(choice == 3)//3BV汇总统计
	{
		...
		printf("\rseed=%d 用时：%d 平均用时：%.1f us\n", seedMax, t1-t0, (float)(t1-t0)/(seedMax-seedMin+1)/iterateMax*1000000);
	...
	else if(choice == 4)//可解种子搜索效率
	{
		...
		printf("\rseed=%d 可解数：%d 用时：%d", seedMax, count, t1-t0);
		printf(" 平均用时：%.1f ms\n", (float)(t1-t0)/count*1000);
		showCursor(1);
	}
			...
			for(iterator=1; iterator<=iterateMax; iterator++)
			{
				...
				if(temp == 1)
				{
					...
					printf("Map:%d*%d-%d,%d,%d,%d,%d,%d\n",
						heightOfBoard, widthOfBoard, numberOfMine, seed, r0, c0, summonMode, iterator);
					printf("3BV=%d\n", bbbv);
					isSolvable = IsSolvableMap(r0, c0);
					printf("IsSolvableMap=%d\n", isSolvable);
					ShowBoard(1);
					//system("pause");
					count++;
	...
	else if(choice == 6)//地图生成效率
	{
		...
		printf(" 平均用时：%.1f us\n", (float)(t1-t0)/(seedMax-seedMin+1)/iterateMax*1000000);
		showCursor(1);
	}
	else if(choice == 7)//种域可解性
	{
		int count = 0;
		t0 = time(0);
		showCursor(visibleCursor);
		for(seed=seedMin; seed<=seedMax; seed++)
		{
			...
			{
				SummonBoard(seed, r0, c0, summonMode, iterator == 1);
				if(IsSolvableMap(r0, c0)) break;
				else if(iterator == iterateMax) count++;
				t1 = time(0);
				if(t2 != t1)
				{
					printf("\rseed=%d 无解数：%d 用时：%d", seed, count, t1-t0);
					t2 = t1;
				}
			}
		}
		printf("\rseed=%d 无解数：%d 用时：%d\n", seedMax, count, t1-t0);
		showCursor(1);
	}
	else if(choice == 8)//调整式可解地图生成效率
	{
		asmcSolve = 0;
		asmcChise = 0;
		asmcRearrange = 0;
		...
		for(seed=seedMin; seed<=seedMax; seed++)
		{
			AdjustSolvableMap(seed, r0, c0);//debug1不显示过程信息，debug2显示
			t1 = time(0);
			printf("\rseed=%d 用时：%d", seed, t1-t0);
		}
		printf(" 平均用时：%.1f ms\n", (float)(t1-t0)/(seedMax-seedMin+1)*1000);
		printf("求解：%d 凿墙：%d 重排：%d 吸雷：%d\n", asmcSolve, asmcChise, asmcRearrange, asmcSwing);
		showCursor(1);
	}
}
...
void ReallocMemory(int height, int width, int dic, int length)//全局矩阵内存重分配
{
	...
	if(height != lastHeight || width != lastWidth)//重分配H*W矩阵
	{
		if(lastHeight != 0 && lastWidth != 0)//释放内存
		{
			...
			isSolved =(int**) MatrixMemory((void**)isSolved, lastHeight, lastWidth, sizeof(int), 0);
		...
		if(height != 0 && width != 0)//申请内存
		{
			...
			isSolved =(int**) MatrixMemory((void**)isSolved, height, width, sizeof(int), 1);
...
struct Records RecordsEditer(struct Records records)//记录编辑器模块
{
	...
	while(1)
	{
		...
		else if(choice == 9)//插入一条记录
		{
			...
			SummonBoard(newRecord.seed, newRecord.r0, newRecord.c0, summonCheckMode, 1);
			newRecord.total3BV = BBBV(1);
			printf("当前地图3BV：%d\n", newRecord.total3BV);
			printf("[已解3BV]>");
...
——新增 通过连续求解加速地图可解性和岛上可解性判断
——新增 调试选项可启用Bench连续求解加速
——新增 地图搜索Debug2支持种域可解性
...
MineSweeper Run 5.21
——优化 逻辑推理效率
——优化 Bench调试不再统计方案数
——优化 Bench调试超过5万局时胜利+已解3BV矩阵分批显示
——优化 操作记录的文件名格式（加入3BV，3BV/s，SS，fail/cheat）
——优化 添加操作记录效率
MineSweeper Run 5.22
——优化 默认关闭NC，启用MC根据剩余雷数排除
——优化 提高地图搜索模块的平均用时显示精度
——优化 游戏时总是尝试添加操作记录以统一手感
——优化 暂停后继续的操作记录
——修复 剩余区雷率可能错误的大于或等于1
——修复 MC存在少量内存泄漏
MineSweeper Run 5.23
——新增 调试选项可启用调整式可解地图（仅自定义难度）
——新增 地图搜索Debug2支持调整式可解地图生成效率
——优化 添加操作记录效率
——修复 可解地图生成效率用时不准确
——修复 种域可解性显示结束种子错误
...
——优化 游戏胜利判断
——优化 可解性判断时序
——优化 地图搜索Debug2功能下调为Debug1
——优化 调整式可解地图生成效率统计更多信息
——优化 游戏时不再调试
——优化 简化枚举限制设置
——修复 未操作的Tab后不使用快速显示
MineSweeper Run 5.25
——新增 筛选可解地图引入调整式可解地图（仅自定义难度）
——优化 修改调整式可解地图算法逻辑以提升性能
——优化 独立根据雷场生成数字函数
——优化 起始点必非空时不再确保地图可解性
——修复 第一次点击直接结束游戏时用时不正确
//——新增 Window操作模式
...
//——新增 游戏信息显示预计用时、SS
//——新增 地图求解可利用帧间信息加速
//——新增 地图求解前置校验（0链打开不全、明显错误标记、明显非法数字）
//——优化 重新设计自定义难度设置，以密度设置雷数不再是调试选项
//——优化 雷率由浮点计算转为整数计算
//——优化 上一次游戏、自制地图的操作记录
//——优化 最大枚举限制由72提高到96，移除NC算法
//——优化 统一游戏操作函数
--------------------------------*/
