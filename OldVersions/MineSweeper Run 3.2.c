...
//int RCScan(int* r, int* c, int p, char key);
		...
		if(choiceMode == 3)
		{
			...
			while(choiceMode == 3 && choiceSet != 7)
			{
				...
				else if(choiceSet == 6)//调试选项
				{
					...
					if(show3BV == 0) printf("(9)启用显示3BV和3BV/s\n");
					if(show3BV == 1) printf("(9)关闭显示3BV和3BV/s\n");
					printf("*******************************\n");
					SetConsoleMode(hdin, ENABLE_QUICK_EDIT_MODE | ENABLE_EXTENDED_FLAGS);
					system("pause");
					//getchar();
...
int ChoiceSetOperateMode(COORD mousePos, DWORD dwButtonState)
{
	...
	else if(IsPosInRectangle(mousePos.X, mousePos.Y, 2, 24, 13, 24))
	{
