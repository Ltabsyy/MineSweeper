...
int main()
{
	...
	if(heightOfBoard == 10 && widthOfBoard == 10 && numberOfMine == 10) difficulty = 0;
	else if(heightOfBoard == 9 && widthOfBoard == 9 && numberOfMine == 10) difficulty = 1;
	else if(heightOfBoard == 16 && widthOfBoard == 16 && numberOfMine == 40) difficulty = 2;
	else if(heightOfBoard == 16 && widthOfBoard == 30 && numberOfMine == 99) difficulty = 3;
	else if(heightOfBoard == 42 && widthOfBoard == 88 && numberOfMine == 715) difficulty = 4;
	else difficulty = 5;
	if(debug == 2)
	{
		printf("[Debug]当前难度：%d\n", difficulty);
	...
	while(choiceMode != 5)
	{
		...
		while(choiceMode == 0)
		{
			...
			else if(rcd.EventType == KEY_EVENT && rcd.Event.KeyEvent.bKeyDown == 1)
			{
				...
				else if(rcd.Event.KeyEvent.wVirtualKeyCode >= 'a' && rcd.Event.KeyEvent.wVirtualKeyCode <= 'g')
				{
			...
			if(lastMap == 0 && summonCheckMode > 2)//可解地图生成
			{
				if(summonCheckMode == 4)//生成可解速通地图
				{
					...
					while(isOpenMine == 1)
					{
						...
						if((difficulty == 0 && bbbv == 2)//默认和初级3BV为2，仅需1次点击
							|| (difficulty == 1 && bbbv == 2)
							|| (difficulty == 2 && bbbv < 50)//中级3BV<50
							|| (difficulty == 3 && bbbv < 160)//高级3BV<160
							|| (difficulty == 4 && bbbv < 1200))//顶级3BV<1200
						{
			...
			while(choiceMode == 1)
			{
				...
				while(choiceMode == 1)
				{
					...
					if(operateMode > 0)
					{
						...
						if(newCursor > 0)
						{
							if(newCursor == 2)//绘制淡黄色高亮光标
							{
								ReShowBoard(r, c, 2, yOfMapEnd-heightOfMapShown+1);
							}
		...
		else if(choiceMode == 3)//设置
		{
			...
			while(choiceMode == 3 && choiceSet != 7)
			{
				...
				else if(choiceSet == 6)//调试选项
				{
					...
					if(dynamicMemory == 0) printf("(9)启用动态内存分配\n");
					if(dynamicMemory == 1) printf("(9)关闭动态内存分配\n");
					printf("*******************************\n");
					SetConsoleMouseMode(0);
					//getchar();
					...
					else if(operation == '7')//设置光标
					{
						...
						if(visibleCursor == 1) printf("(2)隐藏控制台光标\n");
						if(visibleCursor == 0) printf("(2)显示控制台光标\n");
						if(newCursor != 2) printf("(3)启用淡黄色高亮光标\n");
						else printf("(3)关闭淡黄色高亮光标\n");
						printf("(4)退出\n"
							   "*******************************\n");
						printf(">");
						scanf("%d", &temp);
...
void DrawSetDifficulty(int select)
{
	...
	maxHeightOfMap = heightOfWindow-3-4-1;//30-3-4-1=22 42，去除控制栏3，游戏信息栏4，留白1
	...
	printf("**************************************************************\n");
	/*printf("**************************************************************\n");//宽62
	printf("** [(0)默认：10*10 - 10] ** 窗口大小：%3d*%3d 字体大小：%3d **\n", widthOfWindow, heightOfWindow, heightOfFont);
	printf("** [(1)初级： 9*9  - 10] ** 最大地图：%3d*%3d               **\n", maxHeightOfMap, maxWidthOfMap);
...
int ChoiceSetSummonCheckMode(COORD mousePos, DWORD dwButtonState)
{
	...
	else if(IsPosInRectangle(mousePos.X, mousePos.Y, 2, 17, 16, 17))
	{
...
void ShowBlock(int r, int c, int mode)//根据背景颜色和模式显示地图方块
{
	...
	else//深色模式和自定义模式
	{
		...
		else if(mode == 2)//深色模式前台高亮
		{
			...
			{
				/*if(board[r][c] == 0)//以淡黄色0显示空格
				{
					ColorStr("0 ", backgroundColor/16*16 + 0x0e);
				}
				else if(board[r][c] > 5)//深色模式678以淡黄色显示(与前台一致)
				{
					PrintCell(board[r][c], isShown[r][c], backgroundColor/16*16 + 0x0e - 0x04);
				}
				else
...
int GamerLevel(struct Records records)//计算玩家等级并显示称号
{
	...
	if((records.minimumTime[0] != -1 && records.minimumTime[0] <= 13)//默认基准12.80秒
		...
		|| (records.minimumTime[0] != -1 && records.minimumTime[3] <= 344))//343.14秒内赢高级地图
	{
		level = 4;//"Gamer***"
	}
	//完成所有条件获取
		...
		&& records.minimumTime[2] != -1 && records.minimumTime[3] != -1)
	{
		...
		{
			level = 5;//"ProGamer"(Ltabsyy: 5 7 52 170 1656)
	...
	//显示称号
	if(level == 1) printf("Gamer");
	else if(level == 2) printf("Gamer*");
	else if(level == 3) printf("Gamer**");
	else if(level == 4) printf("Gamer***");
	else if(level == 5) printf("ProGamer");
	else if(level == 6) printf("ProGamer*");
	else if(level == 7) printf("ProGamer**");
	else if(level == 8) printf("ProGamer***");
	else if(level == 9) printf("Miner");
	else if(level == 10) printf("Miner*");
	else if(level == 11) printf("Miner**");
	else if(level == 12) printf("Miner***");
	else printf("User");
	return level;
}
