	...
	while(choiceMode != 5)
	{
		...
		while(choiceMode == 0)
		{
			...
			if(rcd.EventType == KEY_EVENT && rcd.Event.KeyEvent.bKeyDown == 1)
			{
				...
				{
					choiceMode = rcd.Event.KeyEvent.wVirtualKeyCode-'1'+1;//主键盘1-7
				}
				if(rcd.Event.KeyEvent.wVirtualKeyCode >= 'a' && rcd.Event.KeyEvent.wVirtualKeyCode <= 'g')
				{
					choiceMode = rcd.Event.KeyEvent.wVirtualKeyCode-'a'+1;//小键盘1-7
				}
			}
			Sleep(refreshCycle);
		}
					...
					if(kbhit())//按键
					{
						...
						if(operateMode == 0)
						{
							if(key == '@' || key == '#' || key == '$')
							{
								...
								//printf(">%c ", operation);
								//scanf("%d%d", &r, &c);
								RCScan(&operation, &r, &c, yOfMapEnd+4);
								break;
							}
						}
		...
		if(choiceMode == 4)//Bench
		{
			clrscr();
			DrawControlBar(0);
			gotoxy(0, 1);
			printf("**                   请等待Bench结束. . .                   **\n");
			Bench(0, 99, heightOfBoard/2, widthOfBoard/2);//Bench模块
			choiceMode = 0;
		}
		if(choiceMode == 6)//地图搜索
		{
			SetConsoleMouseMode(0);
			printf("当前时间戳：%d\n", time(0));
...
void Bench(int seedMin, int seedMax, int r0, int c0)//Bench模块
{
	...
	if(debug == 1 || debug == 2)
	{
		...
		for(seed=seedMin; seed<=seedMax; seed++)
		{
			...
			{
				for(i=0; i<maxPlaceOfBBBV-BBBVOfSeed[seed-seedMin][1]; i++)
				{
					printf(" ");
				}
			...
			{
				for(i=0; i<maxPlaceOfBBBV-BBBVOfSeed[seed-seedMin][0]; i++)
				{
					printf(" ");
				}
...
struct Records RecordsEditer(struct Records records)//记录编辑器模块
{
	...
	while(1)
	{
		...
		if(choice == 1)//删除一条记录
		{
			printf("[序号]>");
			scanf("%d", &i);
			if(records.record[i].difficulty != 5 && records.record[i].isHelped == 0
		...
		else if(choice == 2)//删除无效记录
		{
			...
			{
				if(records.record[i].difficulty == 5 || records.record[i].isHelped == 1
		...
		else if(choice == 3)//仅保留纪录
		{
			...
			{
				if(records.record[i].difficulty == 5 || records.record[i].isHelped == 1
					|| records.record[i].solved3BV != records.record[i].total3BV//无效记录
