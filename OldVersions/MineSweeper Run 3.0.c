...
void DrawBoard(int mode, int remainder, int time);//绘制总外部窗口*/
...
int main()
{
	...
	while(choiceMode != 5)
	{
		...
		/*if(choiceMode == 1 && operateMode == 3)//在外部窗口进行游戏
		{
			int xm, ym;
			mouse_msg mouseMsg;
			//创建窗口
			...
			//ege_enable_aa(true);
			//生成地图
			SummonBoard(heightOfBoard/2, widthOfBoard/2);
			isShown[heightOfBoard/2][widthOfBoard/2] = 1;
			OpenZeroChain(heightOfBoard/2, widthOfBoard/2);
			//游戏循环
			while(1)
			{
				...
				while(mousemsg())
				{
					...
					{
						mousepos(&xm, &ym);
						if(isShown[(ym-heightOfBar)/heightOfBlock][xm/widthOfBlock] != 1)
						{
							isShown[(ym-heightOfBar)/heightOfBlock][xm/widthOfBlock] = 1;
							OpenZeroChain((ym-heightOfBar)/heightOfBlock, xm/widthOfBlock);
						}
					}
					if(mouseMsg.is_right() && mouseMsg.is_up())//鼠标右键抬起
					{
						mousepos(&xm, &ym);
						if(isShown[(ym-heightOfBar)/heightOfBlock][xm/widthOfBlock] == 0)
						{
							isShown[(ym-heightOfBar)/heightOfBlock][xm/widthOfBlock] = 2;
						}
						else if(isShown[(ym-heightOfBar)/heightOfBlock][xm/widthOfBlock] == 2)
						{
							isShown[(ym-heightOfBar)/heightOfBlock][xm/widthOfBlock] = 0;
						}
					}
				}
				Sleep(10);
...
void DrawBoard(int mode, int remainder, int time)//绘制总外部窗口
{
	...
	setcolor(RED);
	setfontbkcolor(BLACK);
	xyprintf((widthOfBoard-5)*widthOfBlock+xOfChar, (heightOfBar-heightOfChar)/2, " %d ", time);
}
*/
void OpenZeroChain(int r0,int c0)//翻开0连锁翻开
{
