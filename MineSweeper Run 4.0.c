...
void RCScan(char* operation, int* r, int* c, int yOfCommand, int showInformation);
				...
				while(choiceMode == 1)
				{
					...
					if(operateMode > 0)
					{
						...
						if(newCursor > 0)
						{
							...
							{
								ReShowBoard(-1, -1, 0, yOfMapEnd-heightOfMapShown+1);
								ReShowBoard(r, c, 2, yOfMapEnd-heightOfMapShown+1);
								//ReShowBoard(r, -1, 2, yOfMapEnd-heightOfMapShown+1);
								//ReShowBoard(-1, c, 2, yOfMapEnd-heightOfMapShown+1);
							}
...
void RCScan(char* operation, int* r, int* c, int yOfCommand, int showInformation)
{
	...
	int refreshCycle = information.r0;*/
	...
	while(isSolving == 1)
	{
		...
		if(isSolving == 0)//策略一二无法进行
		{
			...
			for(r=0; r<heightOfBoard; r++)
			{
				for(c=0; c<widthOfBoard; c++)
				{
					if(isShown[r][c] == 0 && isThought[r][c] < 2 && NumberOfNumberAround(r, c) != 0)
					{
...
void PrintRecords(struct Records records)
{
	...
	//printf("最大翻开数字：%d\n", MaxOpenNumber(records));//现代CPU这么快么？
...
//——优化 逻辑推理的连续操作，枚举判断的剪枝算法，支持无限枚举
//——优化 难度选择界面显示窗口大小，字体大小，最大地图大小
