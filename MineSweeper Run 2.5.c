...
int WASDMove(int* r, int* c, WORD wVirtualKeyCode);//暂时不可用
void DrawSettingsBar(int select);
int ChoiceSettingsBar(COORD mousePos, DWORD dwButtonState);
void DrawDebugOptionsBar(int select);
int ChoiceDebugOptionsBar(COORD mousePos, DWORD dwButtonState);*/
...
// 功能模块
void Solution();
void Bench(int seedMin, int seedMax, int r0, int c0);
...
int operateMode = 1;//操作模式，0@#rc，1wasd23，2鼠标点击
...
int main()
{
	...
	int t0, t1, t2, yOfMapEnd, ro, co;//显示用时，定位，上一个操作坐标
	int set, temp;//设置
	...
	while(choiceMode != 5)
	{
		...
		if(choiceMode == 3)
		{
			...
			DrawSettingsBar(0);
			choiceSet = 0;
			while(choiceMode == 3 && choiceSet == 0)
			{
				ReadConsoleInput(hdin, &rcd, 1, &rcdnum);
				if(rcd.EventType == MOUSE_EVENT)
				{
	...
	while(1)//main内循环防止变量重复定义
	{
		...
		scanf("%d", &choiceMode);
		/*--游戏--*/
		if(choiceMode == 1 || (choiceMode == 0 && lastMap == 1))
		{
			/*重置*/
			clrscr();
			...
			if(choiceMode == 1 && lastMap == 1)//删除文件
			{
				remove("minesweeper-lastmap.txt");
				lastMap = 0;
			}
			if(lastMap == 0)//无上一次地图正常生成
			{
				ShowBoard(0);
				printf("选择坐标[r:行][c:列]\n");
				printf(">@ ");
				...
				else if(operateMode == 1 || operateMode == 2)
				{
					r0 = heightOfBoard/2;
					c0 = widthOfBoard/2;
					if(operateMode == 2) SetConsoleMode(hdin, ENABLE_MOUSE_INPUT | ENABLE_EXTENDED_FLAGS);
					while(1)//简易版wasd23
					{
						if(operateMode == 2)
						{
							...
							{
								mousePos = rcd.Event.MouseEvent.dwMousePosition;
								if(IsPosInRectangle((mousePos.X-cs0)/2, mousePos.Y-rs0, 0, 0, widthOfBoard-1, heightOfBoard-1))
								{
									r0 = mousePos.Y-rs0;
									c0 = (mousePos.X-cs0)/2;
							...
							if(rcd.EventType == KEY_EVENT && rcd.Event.KeyEvent.bKeyDown == 1)
							{
								...
								if(rcd.Event.KeyEvent.wVirtualKeyCode == 'W'
									|| rcd.Event.KeyEvent.wVirtualKeyCode == VK_UP)
								{
									if(r0 > 0) r0--;
								}
								else if(rcd.Event.KeyEvent.wVirtualKeyCode == 'A'
									|| rcd.Event.KeyEvent.wVirtualKeyCode == VK_LEFT)
								{
									if(c0 > 0) c0--;
								}
								else if(rcd.Event.KeyEvent.wVirtualKeyCode == 'S'
									|| rcd.Event.KeyEvent.wVirtualKeyCode == VK_DOWN)
								{
									if(r0 < heightOfBoard-1) r0++;
								}
								else if(rcd.Event.KeyEvent.wVirtualKeyCode == 'D'
									|| rcd.Event.KeyEvent.wVirtualKeyCode == VK_RIGHT)
								{
									if(c0 < widthOfBoard-1) c0++;
								}
								else
								{
									break;
			...
			if(lastMap == 0 && debug == 2)
			{
				set = 0;
				printf("[Debug]seed=%d,%d,%d\n", seed, r0, c0);
				if(operateMode == 2) system("pause");//system指令使SetConsoleMode失效
			...
			while(1)
			{
				...
				while(1)
				{
					if(operateMode == 2)
					{
						...
						ReadConsoleInput(hdin, &rcd, 1, &rcdnum);
						//PeekConsoleInput(hdin, &rcd, 1, &rcdnum);
						//FlushConsoleInputBuffer(hdin);
						if(rcd.EventType == MOUSE_EVENT)
						{
							mousePos = rcd.Event.MouseEvent.dwMousePosition;
						...
						if(rcd.EventType == KEY_EVENT && rcd.Event.KeyEvent.bKeyDown == 1)//按下而非松开键盘时
						{
							...
							if(rcd.Event.KeyEvent.wVirtualKeyCode == 'W'//W/w
								|| rcd.Event.KeyEvent.wVirtualKeyCode == VK_UP)//方向键上
							{
								if(r > 0) r--;
							}
							else if(rcd.Event.KeyEvent.wVirtualKeyCode == 'A'
								|| rcd.Event.KeyEvent.wVirtualKeyCode == VK_LEFT)
							{
								...
								|| rcd.Event.KeyEvent.wVirtualKeyCode == VK_DOWN)
							{
								if(r < heightOfBoard-1) r++;
							}
							else if(rcd.Event.KeyEvent.wVirtualKeyCode == 'D'
								|| rcd.Event.KeyEvent.wVirtualKeyCode == VK_RIGHT)
							{
								if(c < widthOfBoard-1) c++;
							}
		...
		else if(choiceMode == 3)
		{
			while(1)
			{
				...
				if(backgroundColor != 0x07) printf("(4)深色模式\n");
				printf("(5)设置默认操作模式\n"
					   "(6)调试选项\n"
					   "(7)返回菜单\n"
					   "*******************************\n");
				...
				else if(set == 2)//设置地图生成校验
				{
					...
					if(summonCheckMode < 0) summonCheckMode = 0;
					if(summonCheckMode > 3) summonCheckMode = 3;
				}
				...
				else if(set == 5)//设置默认操作模式
				{
					...
					printf("*******************************\n");
					printf("当前默认操作模式：");
					if(operateMode == 0) printf("(0)@#rc\n");
					if(operateMode == 1) printf("(1)wasd23\n");
					if(operateMode == 2) printf("(2)鼠标点击\n");
					printf(">");
					scanf("%d", &set);
					if(set == 0 || set == 2)
					{
