...
struct EnumResults//全局未知链
{
	struct EnumResult* thinkChain;//未知链数组
...
//int countLengthOfThinkChain[128] = {0};
	...
	while(choiceMode != 5)
	{
		...
		if(choiceMode == 1)//游戏
		{
					...
					if(operateMode == 2)
					{
						...
						if(rcd.EventType == MOUSE_EVENT)
						{
							...
							if(IsPosInRectangle((mousePos.X-cs0)/2, mousePos.Y-yOfMapEnd+heightOfBoard-1, 0, 0, widthOfBoard-1, heightOfBoard-1))
							{
								...
								c = (mousePos.X-cs0)/2;
								if((rcd.Event.MouseEvent.dwButtonState == FROM_LEFT_1ST_BUTTON_PRESSED && touchLeftSign == 0)
			...
			else if(choiceMode == 0)//完成地图退出
			{
				...
				//if(operateMode == 3) CloseWindow(temp == 0, remainder, t1-t0+t2);
			}
		}
		...
		else if(choiceMode == 3)//设置
		{
			...
			while(choiceMode == 3 && choiceSet != 9)
			{
				...
				else if(choiceSet == 7)//设置光标
				{
					...
					while(choiceMode == 3 && choiceSet == 7 && choiceTemp == 0)
					{
						...
						else if(rcd.EventType == KEY_EVENT && rcd.Event.KeyEvent.bKeyDown == 1)
						{
							...
							{
								temp = rcd.Event.KeyEvent.wVirtualKeyCode-'1'+1;//主键盘1-4
							}
							else if(rcd.Event.KeyEvent.wVirtualKeyCode >= 'a' && rcd.Event.KeyEvent.wVirtualKeyCode <= 'd')
							{
								temp = rcd.Event.KeyEvent.wVirtualKeyCode-'a'+1;//小键盘1-4
							}
						}
						if(temp == 1)//控制台光标
						{
				...
				else if(choiceSet == 8)//调试选项
				{
					...
					printf("(8)设置Bench\n");
					if(touchMode == 0) printf("(9)启用鼠标点击空地切换左右键\n");
					if(touchMode == 1) printf("(9)关闭鼠标点击空地切换左右键\n");
					printf("*******************************\n");
					SetConsoleMouseMode(0);
					//getchar();
					...
					else if(operation == '2')//设置求解模式
					{
						printf("*******************************\n"//宽31
							   ...
							   "(3)退出\n"
							   "*******************************\n");
						printf("当前模式：%d|枚举限制：NC%dMC%d\n", solveMode, lengthOfThinkNumberCheck, lengthOfThinkMineCheck);
						printf(">");
						scanf("%d", &temp);
						if(temp == 1)
						{
						...
						else if(temp == 2)
						{
							...
							scanf("%d", &temp);
							if(temp >= lengthOfThinkNumberCheck && temp <= LimLength) lengthOfThinkMineCheck = temp;
						}
					}
					...
					else if(operation == '9')//鼠标点击空地切换左右键
					{
...
void DrawSetCursor(int select)
{
	...
	{
		gotoxy(2, 18);
		ColorStr("[(4)退出]", 0xff-backgroundColor);
	}
	gotoxy(0, 20);
}
...
int ChoiceSetCursor(COORD mousePos, DWORD dwButtonState)
{
	...
	else if(IsPosInRectangle(mousePos.X, mousePos.Y, 2, 18, 10, 18))
	{
		if(lastChoice != 4)
		{
			DrawSetCursor(4);
	...
	if((isFoundOpen == 0 || solveMode == 2) && isThinking == 1)//游戏模式已有翻开解不枚举
	{
		...
		while(isSolving == 1)
		{
			...
			if(isSolving == 1)
			{
				...
				if(solveMode != 2 && isFoundOpen == 1) break;//非分析模式有翻开解立即停止
			}
		}
	}
...
int DeepThink()//枚举
{
	...
	int totalNumberOfPossibility, realNumberOfPossibility, temp;//枚举判断
	int isFoundOpen, remainedMine, remainedNotShown;//根据剩余雷数排除(局部)
	...
	if(numberOfThought <= lengthOfThinkNumberCheck)
	{
		...
		totalNumberOfPossibility = ThinkMine(numberOfThought);
	}
	else
	{
		...
		//debug = 2;
		//clrscr();
		totalNumberOfPossibility = ThinkNumber(numberOfThought);
		//debug = 0;
	}
	if(totalNumberOfPossibility > dictionaryNeeded) dictionaryNeeded = totalNumberOfPossibility;
	/*if(totalNumberOfPossibility == 0)
	{
	...
	/*处理字典*/
	realNumberOfPossibility = totalNumberOfPossibility;
	if(totalNumberOfPossibility > LimDictionary)
	{
		if(debug == 1 || debug == 2) printf("[Debug]字典大小不足%d！\n", totalNumberOfPossibility);
		totalNumberOfPossibility = LimDictionary;
		countOfDictionaryOverflow++;//Bench统计
	}
	if(debug == 2)
	{
		printf("[Debug]已生成字典，共%d种可能\n", totalNumberOfPossibility);
		if(totalNumberOfPossibility < 1024)//仅可能性较少时输出调试信息
		{
			for(i=0; i<totalNumberOfPossibility; i++)
			{
				printf("%3d: ", i);
	...
	for(c=0; c<numberOfThought; c++)
	{
		if(sumDictionary[c] == 0) isFoundOpen = 1;
	}
	minNumberOfMine = numberOfThought;
	maxNumberOfMine = 0;
	if(isFoundOpen == 0 || solveMode == 2)//局部根据剩余雷数排除
	{
		...
		for(i=0; i<totalNumberOfPossibility; i++)
		{
				...
				|| remainedNotShown < remainedMine - r)//不含未知链的剩余%数小于剩余雷数减未知链中雷数
			{
				for(c=0; c<numberOfThought; c++)
				{
					sumDictionary[c] -= dictionary[i][c];//和字典中排除该可能
				}
		...
		if(realNumberOfPossibility != totalNumberOfPossibility)
		{
			if(debug == 2)
			{
				printf("[Debug]局部根据剩余雷数排除后剩余%d种可能\n", realNumberOfPossibility);
				printf("Sum: ");
				for(c=0; c<numberOfThought; c++)
				{
					printf("%d ", sumDictionary[c]);
	...
	for(i=0; i<numberOfPossibility; i++)//遍历可能性
	{
		...
		if(isWrong == 0)
		{
			...
			if(isWrong == 0)
			{
				...
				{
					if(realNumberOfPossibility < LimDictionary) dictionary[realNumberOfPossibility][c] = possibility[c];
...
{
	int r, c, i;
	struct EnumResults results;
	struct EnumResult result;
	struct EnumResult* resultsTemp;
	...
	{
		// 枚举得到dictionary
		int numberOfThought, realNumberOfPossibility;
		int minNumberOfMine, maxNumberOfMine;
		// 记录入结果缓存
		result.numberOfThought = numberOfThought;
		result.numberOfPossibility = realNumberOfPossibility;
		...
		for(c=0; c<numberOfThought; c++)
		{
			result.sumDictionary[c] = possibility[c];//已计算入possibility
		}
		//记录某可能的链中雷数
		result.sumMineOfPossibility =(int*) calloc(realNumberOfPossibility, sizeof(int));
		...
		//记录全部可能
		result.dictionary =(int**) MatrixMemory(NULL, realNumberOfPossibility, numberOfThought, sizeof(int), 1);
		for(i=0; i<realNumberOfPossibility; i++)
		{
			for(c=0; c<numberOfThought; c++)
			{
		...
		//存入整体结果
		resultsTemp =(struct EnumResult*) calloc(results.numberOfThinkChain+1, sizeof(struct EnumResult));
		for(i=0; i<results.numberOfThinkChain; i++)
		{
			resultsTemp[i] = results.thinkChain[i];
		}
		resultsTemp[results.numberOfThinkChain] = result;
		if(results.numberOfThinkChain != 0) free(results.thinkChain);
		results.thinkChain = resultsTemp;
		results.numberOfThinkChain++;
		results.minNumberOfPossibleMine += minNumberOfMine;
		results.maxNumberOfPossibleMine += maxNumberOfMine;
	}
	// 是否拼接
	int realNumberOfNotShown;
	r = numberOfMine - NumberOfSign();//暂存未标记雷数，避免重复计算
	realNumberOfNotShown = NumberOfNotShown();
	...
	for(i=0; i<results.numberOfThinkChain; i++)
	{
		if(results.thinkChain[i].minNumberOfMine == results.thinkChain[i].maxNumberOfMine)
		{
			continue;//跳过雷数确定链
		}
	...
	for(i=0; i<results.numberOfThinkChain; i++)
	{
		result = results.thinkChain[i];
		MatrixMemory((void**)result.thinkChain, heightOfBoard, widthOfBoard, sizeof(int), 0);
		MatrixMemory((void**)result.dictionary, result.numberOfPossibility, result.numberOfThought, sizeof(int), 0);
		free(result.sumDictionary);
		free(result.sumMineOfPossibility);
		free(results.thinkChain);
	}
	return 0;
}
*/
...
void Bench(int seedMin, int seedMax, int r0, int c0, int showStep, int showSolution)//Bench模块
{
	...
	if(debug == 1 || debug == 2)
	{
		...
		/*for(i=0; i<128; i++)
		{
			printf("(%d)%d,", i, countLengthOfThinkChain[i]);
			if((i+1)%10 == 0) printf("\n");
		}*/
		printf("胜利矩阵：\n");
		for(seed=seedMin; seed<=seedMax; seed++)
...
void ReallocMemory(int height, int width, int mode)//全局矩阵内存重分配，1申请，0释放
{
	...
	questionMark =(int**) MatrixMemory((void**)questionMark, height, width, sizeof(int), mode);
	...
	//possibility =(int*) calloc(LimLength, sizeof(int));
	...
	mineCheck =(int**) MatrixMemory((void**)mineCheck, height, width, sizeof(int), mode);
	dictionary =(char**) MatrixMemory((void**)dictionary, LimDictionary, LimLength, sizeof(char), mode);
...
——优化 区分和字典和全字典
——优化 NC不再根据剩余雷数排除，枚举后统一进行局部剩余雷数判断
