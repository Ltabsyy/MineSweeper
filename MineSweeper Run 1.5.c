	...
	while(1)//main内循环防止变量重复定义
	{
		...
		if(choiceMode == 1)
		{
			...
			while(1)
			{
				...
				/*显示*/
				system("cls");
				yOfMapEnd = heightOfBoard+Place(widthOfBoard-1)-1;
				if(debug == 1 || debug == 2)
				{
					printf("<Debug>\n");
					printf("Map:%d*%d-%d\n", heightOfBoard, widthOfBoard, numberOfMine);
					...
					printf("\n");
					yOfMapEnd += 6+1*heightOfBoard+Place(widthOfBoard-1);
				}
				ShowBoard(0);
				printf("剩余雷数: %d\n", remainder);//打印剩余雷数
				t1 = time(0);
				...
				if(showTime == 1)
				{
					...
					while(1)
					{
						...
						{
							operateMode = getch();
							if(operateMode == '@' || operateMode == '#'
							|| operateMode == '!' || operateMode == '$')
							{
								//gotoxy(1, yOfMapEnd+5);
								printf("%c", operateMode);
								scanf("%d%d", &r, &c);
								break;
							}
						}
