...
void RCScan(char* operation, int* r, int* c, int yOfCommand);
...
//void ShowBlock(int r, int c, int mode);
void ShowBoard(int mode);//在控制台显示地图，0前台，1后台
//void ShowPartBoard(int r1, int c1, int r2, int c2, int mode);
//void ReShowLineOfBoard(int r, int c, int mode, int yOfMap);
...
int BBBV(int seed, int r0, int c0, int mode);//Bechtel'sBoardBenchmarkValue，计算地图3B
//void** MatrixMemory(void** matrix, int rm, int cm, int sizeOfElements, int mode);//矩阵内存操作
//void ReallocMemory(int mode);//全局矩阵内存重分配，1申请，0释放
void Error();//错误，目前无用
...
int main()
{
	...
	bool touchMode = 0;//鼠标点击模式左右键功能切换，用于触屏标记
	bool isHelped = 0;//记录一局游戏是否被实时求解指令帮助
	// Window
	//int xm, ym;
	//mouse_msg mouseMsg;
	...
	while(choiceMode != 5)
	{
			...
			while(choiceMode == 1)
			{
				...
				if(isShown[r][c] == 1)//对翻开操作做出反应
				{
					...
					else if(board[r][c] == 9)//寄
					{
						...
						if(show3BV == 1)
						{
							printf("3BV：%d/%d\n3BV/s：%.2f\n", bbbv-temp, bbbv, (float)(bbbv-temp)/(t1-t0+t2));
						}
				...
				while(choiceMode == 1)
				{
					...
					if(operateMode == 2)
					{
						...
						if(rcdnum == 0)//无输入时刷新
						{
							...
							if(show3BV == 1)
							{
								gotoxy(0, yOfMapEnd+showTime+2);
								t1 = time(0);
								printf("3BV：%d/%d\n3BV/s：%.2f", bbbv-temp, bbbv, (float)(bbbv-temp)/(t1-t0+t2));
							}
							//gotoxy(1, yOfMapEnd+showTime+2*show3BV+4);//操作指令位置
						...
						if(rcd.EventType == MOUSE_EVENT)
						{
							...
							{
								gotoxy(0, yOfMapEnd+showTime+2*show3BV+3);
							...
							else if(IsPosInRectangle(mousePos.X, mousePos.Y, 0, yOfMapEnd+showTime+2*show3BV+3, 24, yOfMapEnd+showTime+2*show3BV+3))
							{
								if(rcd.Event.MouseEvent.dwButtonState == FROM_LEFT_1ST_BUTTON_PRESSED)
								{
						...
						if(rcd.EventType == KEY_EVENT && rcd.Event.KeyEvent.bKeyDown == 1)//按下而非松开键盘时
						{
							...
							{
								gotoxy(0, yOfMapEnd+showTime+2*show3BV+5);
								if(debug == 1 || debug == 2)
								{
									printf("正在切换到键盘操作...\n");
									system("pause");
					...
					if(kbhit())//按键
					{
						...
						{
							gotoxy(0, yOfMapEnd+showTime+2*show3BV+4);
							printf(">        ");//覆写@ 128 93
							if(debug == 1 || debug == 2)
							{
								printf("\n:(\n检测到@#rc操作指令！正在识别...\n");
							...
							gotoxy(1, yOfMapEnd+showTime+2*show3BV+4);
							operateMode = 0;
							break;//立即更新操作提示
						}
						...
						if(operateMode == 0)
						{
							if(key == '@' || key == '#' || key == '$')
							{
								...
								//printf(">%c ", operation);
								//scanf("%d%d", &r, &c);
								RCScan(&operation, &r, &c, yOfMapEnd+showTime+2*show3BV+4);
								break;
							}
						}
					...
					if(operation == '!' || operation == '\t')
					{
						ShownModeBak(1);
						LookMap();
						gotoxy(0, yOfMapEnd+showTime+2*show3BV+5);//输出debug信息
		...
		if(choiceMode == 3)
		{
			...
			while(choiceMode == 3 && choiceSet != 7)
			{
				...
				{
					if(showTime == 0 && show3BV == 0)
					{
						showTime = 1;
					}
					else if(showTime == 1 && show3BV == 0)
					{
						show3BV = 1;
					}
					else if(showTime == 1 && show3BV == 1)
					{
						showTime = 0;
					}
					else if(showTime == 0 && show3BV == 1)
					{
						show3BV = 0;
					}
...
int ChoiceSetOperateMode(COORD mousePos, DWORD dwButtonState)
{
	...
	else if(IsPosInRectangle(mousePos.X, mousePos.Y, 2, 24, 12, 24))
	{
...
void ShowBlock(int r, int c, int mode)
{
	if(backgroundColor == 0xf0)//浅色模式
	{
		...
		else if(mode == 0)//前台
		{
			...
			else if(isShown[r][c] == 1 && board[r][c] > 5)//浅色模式678显示淡黄色底纹
			{
		...
		else if(mode == 2)//前台加淡黄色底纹
		{
			...
			else if(board[r][c] == 0 && isShown[r][c] == 1)
			{
				ColorStr("  ", 0xe0);
			}
			else
	...
	else//深色模式和自定义模式
	{
		...
		else if(mode == 0)//前台
		{
			...
			else if(isShown[r][c] == 1 && board[r][c] > 5)//深色模式678以淡黄色显示
			{
...
void Bench(int seedMin, int seedMax, int r0, int c0)//Bench模块
{
	...
	for(seed=seedMin; seed<=seedMax; seed++)
	{
		...
		countOfSolved3BV -= BBBV(seed, r0, c0, 0);//减未解3BV计算已解3BV
	...
	else
	{
		...
		if(showTime == 1) printf("用时：%d\n", t1-t0);
		if(show3BV == 1) printf("3BV：%d/%d\n3BV/s：%.2f\n", countOfSolved3BV, countOf3BV, (float)countOfSolved3BV/(t1-t0));
	}
}
...
void ReallocMemory(int mode)//全局矩阵内存重分配，1申请，0释放，2重新申请
{
	isMine =(int**) MatrixMemory((void**)isMine, heightOfBoard, widthOfBoard, sizeof(int), mode);
	numberOfMineAround =(int**) MatrixMemory((void**)numberOfMineAround, heightOfBoard, widthOfBoard, sizeof(int), mode);
	board =(int**) MatrixMemory((void**)board, heightOfBoard, widthOfBoard, sizeof(int), mode);
	isShown =(int**) MatrixMemory((void**)isShown, heightOfBoard, widthOfBoard, sizeof(int), mode);
	zeroChain =(int**) MatrixMemory((void**)zeroChain, heightOfBoard, widthOfBoard, sizeof(int), mode);
	isShownBak =(int**) MatrixMemory((void**)isShownBak, heightOfBoard, widthOfBoard, sizeof(int), mode);
	
	mapShown =(char**) MatrixMemory((void**)mapShown, heightOfBoard+2, 2*widthOfBoard+5, sizeof(char), mode);
	map =(char**) MatrixMemory((void**)map, heightOfBoard, widthOfBoard, sizeof(char), mode);
	numberShown =(int**) MatrixMemory((void**)numberShown, heightOfBoard, widthOfBoard, sizeof(int), mode);
	solution =(int**) MatrixMemory((void**)solution, heightOfBoard, widthOfBoard, sizeof(int), mode);
	isThought =(int**) MatrixMemory((void**)isThought, heightOfBoard, widthOfBoard, sizeof(int), mode);
	numberTeam =(int**) MatrixMemory((void**)numberTeam, heightOfBoard, widthOfBoard, sizeof(int), mode);
	thinkChain =(int**) MatrixMemory((void**)thinkChain, heightOfBoard, widthOfBoard, sizeof(int), mode);
	possibility =(int*) calloc(32, sizeof(int));
	numberCheck =(int**) MatrixMemory((void**)numberCheck, heightOfBoard, widthOfBoard, sizeof(int), mode);
	dictionary =(int**) MatrixMemory((void**)dictionary, LimDictionary, 32, sizeof(int), mode);
	isMineRate =(float**) MatrixMemory((void**)isMineRate, heightOfBoard, widthOfBoard, sizeof(float), mode);
	if(mode == 0)
	{
		isMine = 0;
...
——修复 翻开标记后剩余雷数显示错误
//——新增 调试选项可启用动态内存分配
//——新增 地图大于42行时支持翻页操作，R上21行，F下21行
//——新增 调试选项可启用在外部窗口进行游戏
//——优化 现在地图求解从外部文件读取地图，界面支持鼠标点击
//——优化 @#rc输入坐标体验（输入过程坐标对应位置显示淡黄色底纹，支持wasd改变坐标）
//——优化 wasd23与鼠标点击模式复用部分代码
//——优化 现在默认隐藏光标，wasd23自动显示光标/显示淡黄色底纹光标
//——优化 现在切键盘操作处无需按任意键
//——优化 通过逻辑推理的地图分割和枚举判断的剪枝算法实现时间复杂度由指数级下降为平方级，并支持无限枚举
//——优化 游戏剩余雷数，用时，3BV信息在一行显示
//——优化 Debug的地图搜索功能独立为MapSearch模块以缩减main函数
//——修复 @#rc输入坐标时用时显示停滞
//——修复 雷率为-0和负数时不视为确定解
--------------------------------*/
