...
int lengthOfThinkChain = 21;//未知链长度
...
int Solve()//程序核心部分(doge)
{
	...
	while(isSolving == 1)
	{
		...
		if(isSolving == 0)//策略一二无法进行
		{
			...
			if(isThinking == 1)
			{
				...
				if(isSolving == 0//策略三无法进行
					&& (isFoundOpen == 0 || solveMode == 2))//游戏模式已有翻开解不枚举
				{
					/*策略四
					枚举判断*/
		...
		if(isSolving == 1)
		{
			...
			if(solveMode == 0 && isFoundOpen == 1) return 1;//简单模式有翻开解立即停止
		}
	}
	/*准备输出*/
...
int DeepThink()//枚举
{
	...
	{
		//printf("\n");
		printf("\r                     ");//覆写[Loading...]8192/8192，21空格
	}
...
void SummonMineRateForNotShown()
{
	...
	/*计算平均剩余雷数和未枚举的未知方块数*/
	minRemainedMine = numberOfMine - NumberOfSign() - maxNumberOfPossibleMine;
	maxRemainedMine = numberOfMine - NumberOfSign() - minNumberOfPossibleMine;
	...
	else
	{
		...
		else if(minRemainedMine < 0)
		{
			isMineRateOfNotShown =(float) maxRemainedMine / realNumberOfNotShown;
		}
		else//趋向翻开内部
		{
			isMineRateOfNotShown =(float) minRemainedMine / realNumberOfNotShown;
		}
	}
	if(debug == 2)
	{
...
int GamerLevel(struct Records records)//计算玩家等级并显示称号
{
		...
		&& records.minimumTime[2] != -1 && records.minimumTime[3] != -1)
	{
			...
			&& records.minimumTime[3] <= 243)//242.64秒内赢高级地图
		{
			...
			{
				level = 6;//"ProGamer*"(Ltabsyy: 3 6 39 139 1621)
