...
int main()
{
	...
	if(file = fopen("minesweeper-lastmap.txt", "r"))
	{
		lastMap = 1;
		fclose(file);
	}
	while(1)//main内循环防止变量重复定义
	{
		...
		if(choiceMode == 1 || (choiceMode == 0 && lastMap == 1))
		{
			...
			/*获取地图生成种子和第一次位置*/
			if(lastMap == 1)//有上一次地图读取
			{
				file = fopen("minesweeper-lastmap.txt", "r");
				fscanf(file, "Map:%d*%d-%d\n", &heightOfBoard, &widthOfBoard, &numberOfMine);
			...
			else//无上一次地图正常生成
			{
				...
				else if(operateMode == 1 || operateMode == 2)
				{
					r0 = heightOfBoard/2;
					c0 = widthOfBoard/2;
					if(operateMode == 2) SetConsoleMode(GetStdHandle(STD_INPUT_HANDLE), ENABLE_MOUSE_INPUT | ENABLE_EXTENDED_FLAGS);//确保win10控制台接收鼠标信息
					while(1)//简易版wasd23
					{
						if(operateMode == 2)
						{
							ReadConsoleInput(GetStdHandle(STD_INPUT_HANDLE), &rcd, 1, &rcdnum);
			...
			while(1)
			{
				...
				r = ro;
				c = co;
				if(operateMode == 2) SetConsoleMode(GetStdHandle(STD_INPUT_HANDLE), ENABLE_MOUSE_INPUT | ENABLE_EXTENDED_FLAGS);
				while(1)
				{
					if(operateMode == 2)
					{
						...
						{
							if(rcd.Event.KeyEvent.wVirtualKeyCode == '1')//按1代替!(Shift+1)
							{
								operation = '!';
							}
		...
		else if(choiceMode == 3)
		{
			while(1)
			{
				...
				else if(set == 6)//调试选项
				{
					...
					printf("(8)设置光标\n");
					if(operateMode != 2) printf("(9)启用鼠标操作\n");
					if(operateMode == 2) printf("(9)关闭鼠标操作\n");
					printf("*******************************\n");
					getchar();
					printf(">");
					operation = getchar();
