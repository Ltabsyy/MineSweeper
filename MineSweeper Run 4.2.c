...
void Bench(int seedMin, int seedMax, int r0, int c0, int showStep);//Bench模块
...
int main()
{
	...
	while(choiceMode != 5)
	{
		...
		while(choiceMode == 0)
		{
			...
			{
				if(rcd.Event.KeyEvent.wVirtualKeyCode >= '1' && rcd.Event.KeyEvent.wVirtualKeyCode <= '8')
				{
					choiceMode = rcd.Event.KeyEvent.wVirtualKeyCode-'1'+1;//主键盘1-8
				}
				else if(rcd.Event.KeyEvent.wVirtualKeyCode >= 'a' && rcd.Event.KeyEvent.wVirtualKeyCode <= 'h')
				{
					choiceMode = rcd.Event.KeyEvent.wVirtualKeyCode-'a'+1;//小键盘1-8
				}
			}
			Sleep(refreshCycle);
		}
		...
		else if(choiceMode == 4)//Bench
		{
			if(debug == 2)
			{
				...
				gotoxy(0, 1);
				printf("**                   请等待Bench结束. . .                   **\n");
				Bench(seedMin, seedMax, heightOfBoard/2, widthOfBoard/2, temp);//Bench模块
				system("pause");//暂停观察
			...
			else
			{
				...
				gotoxy(0, 1);
				printf("**                   请等待Bench结束. . .                   **\n");
				Bench(0, 99, heightOfBoard/2, widthOfBoard/2, 1);//Bench模块
			}
			choiceMode = 0;
		}
...
int GamerLevel(struct Records records)//计算玩家等级并显示称号
{
		...
		&& records.minimumTime[2] != -1 && records.minimumTime[3] != -1)
	{
			...
			&& records.minimumTime[3] <= 243)//242.64秒内赢高级地图
		{
			...
			{
				level = 6;//"ProGamer*"(Ltabsyy: 5 7 45 170 1656)
...
void PrintMap(struct Map map, int showNumber)
{
	...
	for(r=0; r<map.height; r++)
	{
		...
		for(c=0; c<map.width; c++)
		{
			...
			else if(map.board[r][c] < 6 || backgroundColor == 0xf0)
			{
...
void Bench(int seedMin, int seedMax, int r0, int c0, int showStep)//Bench模块
{
	...
	for(seed=seedMin; seed<=seedMax; seed++)
	{
		...
		while(1)
		{
			...
			if((debug == 1 || debug == 2) && showStep > 0)//Bench仅显示方案矩阵
			{
				ShowSolution(8);//yOfMap=8
				gotoxy(0, heightOfMapShown+8);
			}
			printf("                     ");//覆写[Loading...]8192/8192，21空格
			/*实时计数*/
			countOfStep++;
		...
		if(showStep > 1)
		{
			gotoxy(0, heightOfMapShown+8);//防止显示进度条后光标位置变化
			system("pause");
		}
	}
	/*输出结果*/
...
void CustomMapsEditer()
{
	...
	while(choice != 5)
	{
		...
		if(choice == 1)//新建地图
		{
			...
			getchar();
			gets(newMap.name);
			//fgets(newMap.name, LimName, stdin);
			printf("[height] [width]>");
