...
/*
void SummonAdjustSolvableBoard(int seed, int r0, int c0)//调整式可解地图
{
	int r, c, temp;
	//生成起始点必为空地图
	SummonBoard(seed, r0, c0, 2, 1);
	while(1)
	{
		//地图求解到不能推进
		int isSolving = 1;
...
void AddOperation(struct Operations* operations, int mstime, char operation, int r, int c)
{
	...
	{
		if(operation == 'm' && operations->tail->operation == 'm')//连续鼠标操作，高频情况
		{
			if(operations->tail->r == r && operations->tail->c == c)//连续鼠标且坐标相同
			{
				//不保存这一次
				free(newNode);
			}
			else if(operations->tail->mstime == mstime)//连续鼠标且时间相同
			{
				//不保存上一次，鼠标消息间隔式处理，最初时间的坐标已过时
				if(operations->tail->prev != NULL)
				{
					operations->tail->prev->next = newNode;
					newNode->prev = operations->tail->prev;
				}
				else//一般不会发生，仅在Window操作模式游玩上一次地图时发生
				{
					newNode->prev = NULL;
				}
		...
		else if(operation == '?' && operations->r == r && operations->c == c)
		{
			free(newNode);//?方块定位操作与当前坐标相同
		}
		else//保存这一次操作记录
		{
			operations->tail->next = newNode;
			operations->tail = newNode;
			if(operation != 'm')//更新当前方块坐标
			{
				operations->r = r;
				operations->c = c;
			}
